...
//constructor   
    ...
    public void StartUpdate() throws IOException
    {
    	...
    	SeeExit1();
    	//an extra check to check the crowding situation// i think this should be ok timing wise
    	//it's not this should be a separate step?
    	...
    	//checkAge();  	
    	//somehow need to do an additional check> but only on cells that didn't exit the first time
    }
    
    //try in a separate method
   ...
   // this literally isn't working// maybe try doing it the other way. 
    ...
    // but the exit method will mess up if the TCs are allowed to exit when bound
    //eg, DCs will think theu are maxed out. 
    // or try doing the exact same thing as before and making the grid CEll thing. 
    //this extra step has to be separate to the other, since the first step may remove the T cell. 
    // the ngh creator still  put s the t cell in the middle so it's the same. 
  // @ScheduledMethod(start = 1, interval = 1 , priority = 9)
    public void ifCrowdedExit() throws IOException{
     ...
     //this didn't work so now i'm trying a different method. 
    // double value = geometryLayer.get(Math.floor(pt.getX()),Math.floor(pt.getY()),Math.floor(pt.getZ()));
     //i think you have to floor this to grab the actual grid, rather than the space the TC is in. 
   //if (value ==18)
    // {
	// if (getRetention()==0)
	// 	{ //GridPoint pt2 = grid.getLocation(this);
	//	 GridCell<Object>here = new GridCell<Object>(pt,Object.class);
	//	 int s= here.size();
	//	 if (s>1.0) // Extra check to see if two cells per grid. 
	// 		{	System.out.println("Two t cells in the grid size: "+s);
	// 			seeIfExit();}
	// 	}}
  //  }
...
//movement method 1 > to a least crowded space
    ...
    //to iterate over grids. Maybe add to the alterGrid section?
    
    public void checkCrowding(){
    	Context<Object> context = (Context)ContextUtils.getContext(this);
        GridValueLayer geometryLayer = (GridValueLayer)context.getValueLayer("Geometry");
        //for all grids 
        // if contain 2 t cells 
        //see if exit again
    }
